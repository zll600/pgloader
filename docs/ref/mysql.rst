...
keys*, *downcase identifiers*, *uniquify index names*.
  ...
  - *identity columns*
    ...
    When this option is listed, MySQL ``auto_increment`` columns are created
    as PostgreSQL ``GENERATED BY DEFAULT AS IDENTITY`` columns instead of
    using ``serial`` data types. This option requires PostgreSQL version 10
    or newer; on older versions pgloader falls back to ``serial`` and emits
    a warning.
